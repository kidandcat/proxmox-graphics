...
vGraphics = (function(config) {
    ...
    var max = config.max;
    var symbol = '';
    var URL = config.url || 'http://localhost:8000';

    o.createChart = function(data, type, symbol, data2, label2) {
                    ...
                    tooltips: {
                        ...
                        callbacks: {
                            label: function(tooltipItems, data) {
                                return tooltipItems.yLabel + ' ' + symbol;
                            }
                        }
                    },
                },
        ...
        xhttp.onreadystatechange = function() {
            if (xhttp.readyState == 4 && xhttp.status == 200) {
                ...
                var dat = data[Object.keys(data)[0]].data.map(function(ee) {
                    ...
                    switch (type.toUpperCase()) {
                        case 'CPU':
                            symbol = '%';
                            return {
                                y: Math.round(ee.cpu * 100),
                                x: new Date(time5)
                            }
                        case 'DISK':
                            symbol = 'GB';
                            max = Math.floor(ee.maxdisk / 1000000000 * 100) / 100
                            return {
                                y: Math.floor(ee.disk / 1000000000 * 100) / 100,
                                x: new Date(time5)
                            }
                        case 'NET':
                            symbol = 'KB/s';
                            ...
                            return {
                                y: Math.round(ee.netout / 1000 * 100) / 100,
                                x: new Date(time5)
                            }
                        case 'MEMORY':
                            symbol = 'MB';
                            max = Math.floor(ee.maxmem / 1000000)
                            return {
                                y: Math.floor(ee.mem / 1000000),
                                x: new Date(time5)
                            }
                    }
                ...
                if (type.toUpperCase() == 'NET') {
                    ...
                    o.createChart(o.swapY(dat), type, symbol);
                }
            }
        };
    ...
    o.swapY = function(aObj) {
        var yy = [];
        aObj.forEach(function(oo) {
            yy.push(oo.y);
        });
        aObj.forEach(function(oo) {
            oo.y = yy.pop();
